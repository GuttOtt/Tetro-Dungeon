...
public class SkillDescriptor : MonoBehaviour
{
    ...
    public void DescribeSkill(SkillConfig skillConfig)
    {
        string skillName = skillConfig.SkillName;
        string description = RegexDescription(skillConfig, skillConfig.SkillDescription);
    ...
    private string RegexDescription(SkillConfig skillConfig, string template)
    {
        var type = skillConfig.GetType();
        Debug.Log($"{type}");
        return SkillDescriptionRegex.Replace(template, match =>
        {
            string fieldName = match.Groups[1].Value; // 중괄호 안의 값 추출
            ...
            if (property != null)
            {
                return property.GetValue(skillConfig)?.ToString() ?? match.Value;
            }
            return match.Value; // 일치하는 프로퍼티가 없으면 원래 값 반환
        });
    }
